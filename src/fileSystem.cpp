...
void backend::FileSystem::_createFolder(const std::string& folderName) noexcept {
    ...
    if (!std::filesystem::create_directory(_tempPath)) {
        std::cerr << "Error : " << _tempPath
                  << " Folder Not Created Nor Found" << std::endl;
    }
...
void backend::FileSystem::_applyHomeNameTakenFromEnvironment() noexcept {
    std::string tempUserName = getenv("HOME");
    auto stringSize = tempUserName.size();

    for (int i = 0; i < stringSize; ++i) {

        _homeName[i] = tempUserName.at(i);
    }
}

void backend::FileSystem::setup() {

    _cleanCharString(_homeName, UNAME_MAX);
    _applyHomeNameTakenFromEnvironment();

    _cleanCharString(_tempPath, PATH_MAX);
    _cleanCharString(_path, PATH_MAX);

    // Main folder setup
    _appendCharString(_path, PATH_MAX, _homeName, UNAME_MAX);
